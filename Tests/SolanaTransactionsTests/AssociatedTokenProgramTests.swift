...
@Test func createATAAndVerify() async throws {
    ...
    guard let mintPublicKey = PublicKey(bytes: [UInt8](mintPrivateKey.publicKey.rawRepresentation)) else {
        fatalError("Failed to create mint public key")
    }

    let mintAuthorityPrivateKey = Curve25519.Signing.PrivateKey()
    guard let mintAuthorityPublicKey = PublicKey(bytes: [UInt8](mintAuthorityPrivateKey.publicKey.rawRepresentation)) else {
        fatalError("Failed to create mint authority public key")
    }

    let ownerPrivateKey = Curve25519.Signing.PrivateKey()
    guard let ownerPublicKey = PublicKey(bytes: [UInt8](ownerPrivateKey.publicKey.rawRepresentation)) else {
        fatalError("Failed to create owner public key")
    }
    
    print("Mint Public Key: \(try BorshEncoder.base58Encode(mintPublicKey))")
    ...
    try await rpc.requestAirdrop(to: try BorshEncoder.base58Encode(mintAuthorityPublicKey), lamports: UInt64(airdropAmount))
    try await rpc.requestAirdrop(to: BorshEncoder.base58Encode(ownerPublicKey), lamports: UInt64(airdropAmount))

    //get rent exempt balance and recent blockhash
    let rentExemptBalanceResponse = try await rpc.getMinBalanceForRentExemption(accountDataLength: 82)
    var blockhashResponse = try await rpc.getLatestBlockhash()

    print("DEBUG: Got Blockhash \(blockhashResponse.blockhash)")
    let createAndInitializeMintTransaction = try! Transaction(blockhash: blockhashResponse.blockhash) {
    ...
    try await rpc.sendTransaction(transaction: createAndInitializeMintTransaction, options: TransactionOptions(
        skipPreflight: true,
    ...
    try await rpc.sendTransaction(transaction: buildATATransaction, options: TransactionOptions(
        skipPreflight: true,
        preflightCommitment: .confirmed
    ))
